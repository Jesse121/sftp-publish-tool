...
module.exports = function (options) {
	...
	asyncForEach(filesInfo, info => {
		...
		const absRemotePath = path.join(sftpConfig.remotePath, absLocalPath.replace(absBasePath, "")).replace(/\\/g, "/");
		...
		if (info.isDirectory) {
			...
			sftp.fastPut(absLocalPath, absRemotePath).then(() => {
				...
				spinner.text = `传输: ${((currentSize * 100) / size).toFixed(2)}% ${absRemotePath} ${info.size} 字节`;
				if (currentSize === size) {
