...
program
	...
	.option("-e, --environment [name]", "publish environment.")
	.parse(process.argv);

const sftpConfig = getSftpConfig(program.environment);

function handle() {
	const fmtSftpConfig = formatSftpConfig(sftpConfig, program.remove || program.copy);
	const options = {
		sftp,
